...
class userinfoCommand extends SubCommandPluginCommand {
    ...
    async constructUserinfoEmbed(memberObj) {
        ...
        const roles = (memberObj.roles.cache.size === 1) ? 'No roles :(' : Array.from(memberObj.roles.cache.values()).filter(m => m.name !== '@everyone').join(' ');
        const sortedMembers = memberObj.guild.members.cache.sort(this.compareJoinedAt).map(m => m.user);
        ...
        return new MessageEmbed()
            ...
            .addField(
                'Joined Discord on',
                `${memberObj.user.createdAt.toLocaleDateString('en-US', options)}\n${createdDaysAgo}`,
                true
            )
            .addField(
                `Joined ${memberObj.guild} on`,
                `${memberObj.joinedAt.toLocaleDateString('en-US', options)}\n${joinedDaysAgo}`,
                true
            )
            .addField(
                'Roles',
            ...
            .setFooter({
                text: `Member #${sortedMembers.indexOf(memberObj.user) + 1} | User ID: ${memberObj.id}`
            });
    }
